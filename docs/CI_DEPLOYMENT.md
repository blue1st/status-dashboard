...
### 1. ワークフローの構成
ワークフローファイル（`.github/workflows/status-check.yml`）は、1回の実行で以下の2つの主要なタスクを行います：
1. **データの生成**: `npm run generate` を実行して `data/status.json` を一時的に生成します。
2. **Pages へのデプロイ**: 生成された JSON を含むワークスペース全体をアーティファクトとしてアップロードし、デプロイを実行します。

*注: 以前は JSON をリポジトリにコミットしていましたが、ローカル開発時のコンフリクトを避けるため、現在は「アーティファクトベース」のデプロイ（リポジトリへの書き戻しを行わない方式）に変更されています。*
...
| **データの永続化** | アーティファクトのみ | アーティファクトのみ |
| **Bot ユーザー** | (不要になりました) | `gitlab-ci-token` |
| **ループ防止** | (プッシュしないため不要) | `[skip ci]` または専用ロジック |
